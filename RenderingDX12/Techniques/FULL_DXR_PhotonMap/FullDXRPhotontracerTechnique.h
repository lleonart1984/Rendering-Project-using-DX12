...
#define MAX_NUMBER_OF_PHOTONS (DISPATCH_RAYS_DIMENSION*DISPATCH_RAYS_DIMENSION*3)
	...
	struct DXR_PT_Pipeline : public RTPipelineManager {
		...
		class DXR_PT_IL : public DXIL_Library<DXR_PT_Pipeline> {
			void Setup() {
				_ gLoad DXIL(ShaderLoader::FromFile(".\\Techniques\\FULL_DXR_PhotonMap\\FullDXRPhotonTracing_RT.cso"));
	...
	void CreatingAssets(gObj<CopyingManager> manager) {
		...
		PhotonsAABBs = _ gCreate GenericBuffer<D3D12_RAYTRACING_AABB>(D3D12_RESOURCE_STATE_GENERIC_READ, MAX_NUMBER_OF_PHOTONS, CPU_WRITE_GPU_READ, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
