...
bool GenerateFullVariablesWithNewModel(float G, float Phi, float3 win, float density, out float3 x, out float3 w, out float factor, out float3 X, out float3 W)
{
	...
	float logN = density < 0.001 ? 0 : max(0, sampleNormal(lenOutput[0], lenOutput[1]));
	float n = ceil(exp(logN));
	logN = log(n);
	...
	float3 pathMu = float3(pathOutput[0], pathOutput[1], pathOutput[2]);
	float3 pathLogVar = float3(pathOutput[3], pathOutput[4], pathOutput[5]);
	float3 pathOut = pathMu + exp(clamp(pathLogVar, -40, 76) * 0.5) * sampling;
	float costheta = clamp(pathOut.x, -1, 1);
	float wt = pathOut.y;
	float wb = pathOut.z;
	x = float3(0, sqrt(1 - costheta * costheta), costheta);
	float3 N = x;
...
// Will update importance with scattered ratio divided by pdf
// Will output scattered ray to continue with
void VolumeScattering(inout float3 x, inout float3 w, inout float3 importance, float Extinction, float G, float Phi, inout float3 accum)
{
	...
	int mIndex;
	float3 coords;
	if (!Intersect(x, LightDirection, tIndex, mIndex, coords)) // 
	{
		importance = float3(1, 0, 1) * 0;
		//return;
		//return; // bad geometry... 
	}
	...
	accum += importance * LightIntensity / (pi)*exp(-d * Extinction) * EvalPhase(G, w, LightDirection);
	...
	if (!pathTrace) // Try to skip multiple scattering
	{
		...
		if (density > 0 && -log(1 - random()) / density < 1) // multiple scatter really occur
		{
			...
			if (!GenerateFullVariablesWithNewModel(G, Phi, w, density, _x, _w, factor, _X, _W))
				importance = 0;

			float3 sX = x + _X * r;
			if (!Intersect(sX, LightDirection, tIndex, mIndex, coords)) // 
			{
			...
			GetHitInfo(coords, tIndex, mIndex, surfel, material, 0, 0);
			d = length(surfel.P - sX); // distance to surface in direction to light
...
float3 ComputePath(float3 O, float3 D, out int volBounces)
{
	...
	while (importance[cmp] > 0)
	{
		...
		if (!Intersect(x, w, tIndex, mIndex, coords)) // 
			return accum + importance * SampleLight(w) * 0.5 * (bounces == 1);// +importance * (SampleSkybox(w) + SampleLight(w) * (bounces > 0));
		...
		{
			x += t * w;
			VolumeScattering(x, w, importance, Extinction[cmp], G_Value[cmp], Phi[cmp], accum);
		}
	}
