...
bool GetPrimaryIntersection(uint2 screenCoordinates, float2 coordinates, out float3 V, out Vertex surfel, out Material material, out int materialIndex) {
	bool valid = any(Positions[screenCoordinates]);
	...
	material = materials[materialIndex = MaterialIndices[screenCoordinates]];
