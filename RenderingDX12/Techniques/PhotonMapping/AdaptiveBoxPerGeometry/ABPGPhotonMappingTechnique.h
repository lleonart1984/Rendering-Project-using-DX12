...
struct ABPGPhotonMappingTechnique : public Technique, public IHasScene, public IHasLight, public IHasCamera {
public:
	...
	struct ComputeRadiiPipeline : public ComputePipelineBindings {
		gObj<Buffer> Photons;
	...
	struct ComputeAABBsPipeline : public ComputePipelineBindings {
		...
		void Setup() override {
			_ gSet ComputeShader(ShaderLoader::FromFile(".\\Techniques\\PhotonMapping\\AdaptiveBoxPerGeometry\\ABPGAABBConstruction_CS.cso"));
		}
	...
	struct DXR_RT_Pipeline : public RTPipelineManager {
		...
		class DXR_RT_IL : public DXIL_Library<DXR_RT_Pipeline> {
			void Setup() {
				_ gLoad DXIL(ShaderLoader::FromFile(".\\Techniques\\PhotonMapping\\AdaptiveBoxPerPhoton\\ABPPPhotonGather_RT.cso"));
		...
		struct DXR_RT_Program : public RTProgram<DXR_RT_Pipeline> {
			...
			void Globals() {
				...
				SRV(9, Normals);
				SRV(10, Coordinates);
				SRV(11, MaterialIndices);

				SRV(12, LightPositions);

				SRV_Array(13, Textures, TextureCount);
