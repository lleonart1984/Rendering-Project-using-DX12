...
struct BPGPhotonMappingTechnique : public Technique, public IHasScene, public IHasLight, public IHasCamera {
public:
	...
	struct DXR_PT_Pipeline : public RTPipelineManager {
		...
		class DXR_PT_IL : public DXIL_Library<DXR_PT_Pipeline> {
			void Setup() {
				_ gLoad DXIL(ShaderLoader::FromFile(".\\Techniques\\PhotonMapping\\BoxPerGeometry\\BPGPhotonTracer_RT.cso"));
	...
	struct ComputeAABBsPipeline : public ComputePipelineBindings {
		...
		void Setup() override {
			_ gSet ComputeShader(ShaderLoader::FromFile(".\\Techniques\\PhotonMapping\\BoxPerGeometry\\BPGAABBConstruction_CS.cso"));
		}

		void Globals() override {
			SRV(0, Photons, ShaderType_Any);
			SRV(1, HashTable, ShaderType_Any);
	...
	struct DXR_RT_Pipeline : public RTPipelineManager {
		...
		class DXR_RT_IL : public DXIL_Library<DXR_RT_Pipeline> {
			void Setup() {
				_ gLoad DXIL(ShaderLoader::FromFile(".\\Techniques\\PhotonMapping\\BoxPerGeometry\\BPGPhotonGather_RT.cso"));
		...
		struct DXR_RT_Program : public RTProgram<DXR_RT_Pipeline> {
			...
			void Globals() {
				...
				SRV(8, Normals);
				SRV(9, Coordinates);
				SRV(10, MaterialIndices);

				SRV(11, LightPositions);
...
#include "../PhotonDefinition.h"
	...
	void CreatingAssets(gObj<CopyingManager> manager) {
		...
		PhotonsAABBs = _ gCreate RWAccelerationDatastructureBuffer<D3D12_RAYTRACING_AABB>(sceneLoader->VertexBuffer->ElementCount / 3);
...
#pragma region Compute shader pipeline objects
		...
		computeAABBsPipeline->HashTable = dxrPTPipeline->_Program->HashTable;
		computeAABBsPipeline->NextBuffer = dxrPTPipeline->_Program->NextBuffer;
...
#pragma region Photon Trace Stage
		...
		manager gClear UAV(ptRTProgram->HashTable, (unsigned int)-1);
...
#pragma region Update Photon Map BVHs in next frames
		...
		geomUpdater gLoad Geometry(0, sceneLoader->VertexBuffer->ElementCount / 3);
