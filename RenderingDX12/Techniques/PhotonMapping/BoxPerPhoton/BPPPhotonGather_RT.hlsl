...
#ifndef VERTICES_REG
#define VERTICES_REG				t3
#endif

#ifndef MATERIALS_REG
#define MATERIALS_REG				t4
#endif

#ifndef GBUFFER_POSITIONS_REG
#define GBUFFER_POSITIONS_REG		t5
#endif

#ifndef GBUFFER_NORMALS_REG
#define GBUFFER_NORMALS_REG			t6
#endif

#ifndef GBUFFER_COORDINATES_REG
#define GBUFFER_COORDINATES_REG		t7
#endif

#ifndef GBUFFER_MATERIALS_REG
#define GBUFFER_MATERIALS_REG		t8
#endif

#ifndef LIGHTVIEW_POSITIONS_REG
#define LIGHTVIEW_POSITIONS_REG		t9
#endif
...
void PhotonGatheringAnyHit(inout PhotonRayPayload payload, in PhotonHitAttributes attr) {
	...
	if (any(p.Intensity))
		...
		payload.Accum += kernel * 0.5 * float3(0, 0, 0);
...
void PhotonGatheringAnyHit(inout PhotonRayPayload payload, in PhotonHitAttributes attr) {
	...
	if (photonDistance < PHOTON_RADIUS && NdotL > 0.001 && NdotN > 0.001)
	{
		float kernel = 2 * (1 - photonDistance / PHOTON_RADIUS);

		float3 BRDF =
			NdotN;// *DiffuseRatio;
			//1.0 / area;// *DiffuseRatio;
...
#else
	return photonGatherPayload.Accum * material.Roulette.x * material.Diffuse / pi / 100000 / (pi * PHOTON_RADIUS * PHOTON_RADIUS);// +material.Specular * photonGatherPayload.OutSpecularAccum;
#endif
}