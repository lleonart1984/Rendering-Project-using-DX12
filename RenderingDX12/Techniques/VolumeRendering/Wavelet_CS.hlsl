...
float Kernel(int2 currentPx, int sampleID, int2 sampleCoord) {
	...
	float wp = exp(-length(Positions[currentPx].xyz - Positions[sampleCoord].xyz) * (100));
	float wg = DirectionWeight(Gradient[currentPx], Gradient[sampleCoord]);// pow(saturate(dot(Gradient[currentPx], Gradient[sampleCoord])), 2);
